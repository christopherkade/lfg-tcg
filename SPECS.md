...
- **Pre-auth activity signal:** `/login/page.tsx` (a Server Component) also fetches `get_platform_activity_stats()`/`get_recent_platform_activity()` (Section 3's new callout) as the anon role and passes them into `LoginView` as `initialStats`/`initialEvents`, so `PlatformActivityTicker` (rendered between the numbered steps and the Discord button) paints instantly with real "N active now · M matched today" counts plus a rotating recent-event line, instead of a client-side fetch flash. This is the one place in the app that reads Supabase data before a session exists.
...
>
> **`get_profile_pod_stats()` (`supabase/sql/profile_pod_stats.sql`) is a second read-only SECURITY DEFINER RPC, mirroring `get_games_played_count()`'s exact host-or-member/`hidden_by`-ignoring criteria.** It backs the Profile screen's "Pods Hosted"/"IRL Games"/"Online Games" stat cards (Section 5's Screen 1) and returns all three counts (`pods_hosted`, `irl_count`, `online_count`) as a single row — `pods_hosted` is a `count(*) filter (where host_id = auth.uid())` inside the same host-or-member-filtered query rather than a separate query, since every hosted row already satisfies that filter. One RPC call instead of three keeps the Profile page's `Promise.all` from paying for extra round trips.
>
...
- **Stats (`ProfileStats`):** A "Stats" section below the header — a 2x2 grid of four real stat cards plus one full-width card reserved for future trends/graphs. **Games Played** (sourced from `get_games_played_count()`, same RPC the History screen uses, Section 5's Screen 4), **Pods Hosted**, **IRL Games**, and **Online Games** (all three from the single `get_profile_pod_stats()` RPC, Section 3) are passed down already-resolved (`profile/page.tsx` awaits all three via one `Promise.all` alongside `getProfile`, no `Suspense`/`use()` needed since this page already blocks on `getProfile`) and each animated with a shared Framer Motion count-up hook from 0 on mount (skipped under `useReducedMotion()`, showing the final numbers immediately instead). The full-width card below the grid is a dashed/muted placeholder with a "Trends and match history graphs are coming soon" caption — the one remaining piece of this section still genuinely unbuilt. All cards fade/stagger in together on mount (Framer Motion `staggerChildren`), skipped under `useReducedMotion()`.
- **City Field (`ProfileForm`, `CitySelector`):** Since a `profiles` row can only be created by a first username save (see below), `ProfileForm`'s city section (the city-missing alert, `CitySelector`, and its own Save button, submitting to `updateCity`) only renders once `initialProfile` already exists; a brand-new account sees the "Pick a username to start using PodFinder." info `Alert` in its place instead (the same copy that used to sit above the form's now-removed username field). Once a profile exists, `CitySelector` is a config-driven autocomplete (options from `CITIES_CONFIG`, see Section 4) selecting `profiles.city` — optional, since a user who only ever plays Online doesn't need one; this is what lets the Match Feed (Section 5's Screen 3, Section 6) automatically scope IRL pods to the viewer's own city while still always showing every Online pod. Whenever `initialProfile.city` is unset, an info `Alert` ("Add your city to create and join in-person pods near you.") is shown directly above `CitySelector`. Saving redirects to `/` on success, same as before this change.
- **Two focused save actions (`src/app/actions/profile.ts`):** `updateUsername` (validates non-empty, then either `update`s the existing row's `username` or, the very first time, `insert`s a new row with `username`/`discord_handle`/`avatar_url` — `city` and every `preferred_*` column are left to their DB defaults/`NULL` on that first insert, see Section 3) and `updateCity` (validates against `CITY_MAP`, then a plain `update` — it can assume a row already exists, since `ProfileForm`'s city section is gated on that). Splitting these out of the old single `upsertProfile` action reflects that username and city are now two independent save flows with two different UI locations, not one combined form submit.
...
- No chart/graph library or wired-up trends data — the Profile tab's full-width trends card (Section 5's Screen 1) is a deliberate "coming soon" placeholder reserving layout space, not real data. (Its four stat cards above it — Games Played, Pods Hosted, IRL Games, Online Games — are all real.)
