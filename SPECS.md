...
- **Settings button (`SettingsDialog`):** a gear icon button rendered directly to the left of `TutorialDialog` in both the mobile top bar and the desktop navbar (same header row either way). Clicking it opens a modal containing the language preference (`LocaleSwitcher`) and the light/dark theme toggle (`ThemeToggle`) — see Section 1's Theming subsection.
