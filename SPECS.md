...
- **Realtime Subscriptions:** Enabled on the `beacons` and `beacon_joins` tables to push instant matching feeds.
...
Enable Supabase Realtime replication on the `beacons` and `beacon_joins` tables to push instant matching feed and join-request updates.
...
- **Join-request alerts (not push):** `JoinRequestNotifier` uses the browser `Notification` API to show a native OS notification, and an in-app snackbar, when a host's own active beacon receives a new join request, or when a joiner's own request is accepted. It also shows a blocking `MatchedDialog` to accepted members when their beacon is marked `MATCHED` (see Section 5's Join Request Flow, step 5). This only fires while the PWA/tab process is actually running (foreground or backgrounded tab) — there is no service worker or server-triggered push involved, so nothing is delivered while the app is fully closed.
...
13. Add the optional `notes` field to the Search dialog and `beacons` table; add `BeaconDetailDialog` (opened by clicking a Match Feed card) per Section 5; add `JoinRequestNotifier` (native `Notification` + in-app snackbar on new join requests, mounted in the `(app)` layout) per Sections 5 and 7.
...
15. Extend `JoinRequestNotifier` with a `MatchedDialog`, shown to each accepted member (not the host) when their beacon transitions to `MATCHED`, per Section 5's Join Request Flow step 5.
```
